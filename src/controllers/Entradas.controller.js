...
const obtenerProductos = () => {
  ...
  const objetosFilas = tablaArray.map((fila) => {
    ...
    objetoFila.descripcion = columnas[1].textContent;
...
const confirmarCompra = () => {
  ...
  confirmarBtn.addEventListener("click", async () => {
    const confirmado = window.confirm(
      "¿Está seguro de confirmar la resepcion?"
    );
    if (confirmado) {
      const compra = {
        cliente: Cliente,
      ...
      productoList.forEach((producto) => {
        const inventarioReal = catalogoIndexado[producto.codigo];
        const produtoInventario = {
          codigo: producto.codigo,
          cantidad: (inventarioReal.cantidad += producto.cantidad),
        };
